...
public class CipherService  {
    ...
    public void _main(String[] args) throws Exception {
        ...
        final String sampleInput = "obfuscate world!";
    ...
    public void demo(String input) throws Exception {
        ...
        byte[] byteInput = getUTF8Bytes(input);
        byte[] byteOutput = new byte[32];

        //Initializes the cipher with ENCRYPT_MODE, key and iv.
        encipher.init(Cipher.ENCRYPT_MODE, key, iv);
        //Continues a multiple-part encryption/decryption operation for byte array.
        int updateBytes = encipher.update(byteInput, 0, byteInput.length, byteOutput, 0);
        System.out.println(updateBytes);
        //We must call doFinal at the end of encryption/decryption.
        int finalBytes = encipher.doFinal(byteInput, 0, 0, byteOutput, updateBytes);
        System.out.println(finalBytes);
        //Closes the cipher.
        encipher.close();

        System.out.println(Arrays.toString(Arrays.copyOf(byteOutput, updateBytes+finalBytes)));
        ...
        decipher.init(Cipher.DECRYPT_MODE, key, iv);
        byte [] decoded = new byte[32];
        decipher.doFinal(byteOutput, 0, updateBytes + finalBytes, decoded, 0);

        System.out.println("output: " + new String(decoded, StandardCharsets.UTF_8));
    }

    public String encipher(String input) throws Exception {
